...
function read_therm_meas(h5id, name)
    ...
    for (j,repeat) in enumerate(repeats)
        for n in 0:(N_replicas - 1)
            E_meas[:, n + 1, j] = read(h5id, "$name/$repeat/Rep_$n/E_meas")[:, end]
            E_therm[:, n + 1, j] = read(h5id, "$name/$repeat/Rep_$n/E_therm")[:, end]
            E0[n + 1, j] = read(h5id, "$name/$repeat/Rep_$n/S0")[end]
        end
    end
...
function main(file, name, dir;repeat = 1)
    ...
    has_therm = all( [ haskey(h5id, "$name/$r/Rep_$n/E_therm") for r in repeats, n in 0:Nrep-1])
    has_meas  = all( [ haskey(h5id, "$name/$r/Rep_$n/E_meas") for r in repeats, n in 0:Nrep-1])
