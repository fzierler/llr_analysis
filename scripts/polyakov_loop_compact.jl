...
function main(h5file, ens, plot_name)
    ...
    if !is_fixed_a_measured(h5, ens)
        ...
        fig = Figure(size = (600*2, 2*375),fontsize = 18pt)
        save(plot_name, fig) 
        return nothing 
    end
    ...
    @showprogress desc="plot Polyakov loop $ens" for rep_ind in 1:Nint
        ...
        if group == "SU(3)"
            title = L"SU(3),~%$Nt\times%$(Ns)^3,~N_{\mathrm{rep}}=%$Nint"
            ...
            datashader!(ax1,points_im, colormap=[:transparent, :grey, :black], binsize=2)
            ...
            title = L"Sp(4),~%$Nt\times%$(Ns)^3,~N_{\mathrm{rep}}=%$Nint"
            poly_label = L"\text{Re}(\ell_p)"
        ...
        ax2 = Axis(fig[2, 2]; title, xlabel, ylabel = L"a_n", limits = (extrema(up), nothing))
        ...
        # save figure
        save("$(tmp_base)_ind$(rep_ind).pdf",fig)
    end
    tmp_plots = ["$(tmp_base)_ind$(rep_ind).pdf" for rep_ind in 1:Nint]
    merge_pdfs(tmp_plots, plot_name, cleanup=true)
    return nothing
end
...
function main()
    ...
    main(args["h5file"],args["run_name"],args["plot_name"])
end
main()